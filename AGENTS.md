...
| 配置 | `config.json` | ~21 | 持久化配置：`rows`、`cols`、`hotkeys`（名称→转义序列）、`scrollIntervalTerminal`、`scrollIntervalPage`、`screenHistoryLines`（重连滚屏历史行数，默认 1000）、`maxFrontendLogs`、`feishuAppId`/`feishuAppSecret`/`feishuReceiveId`/`feishuReceiveType`（飞书推送） |
...
| `feishu_app_id` | 更新飞书应用 app_id |
| `feishu_app_secret` | 更新飞书应用 app_secret |
| `feishu_receive_id` | 更新飞书消息接收 ID |
| `feishu_receive_type` | 更新飞书接收 ID 类型（email/chat_id/open_id/user_id） |
...
41. **完成标记触发通知通道（原 BEL 通道 2026-07-14 引入，2026-07-22 改为完成标记触发）**：用显式完成标记（默认 `REMOTECMD_DONE`）替换原 BEL(`\x07`) 检测作为通知触发源——BEL 被 TUI/程序自发响铃误报太多。命中后前端显示 `终端通知: <终端名>` Toast + 播放 1kHz 蜂鸣（默认 300ms，可配）+ 系统 OS 弹窗 +（可选）ntfy 手机推送 / 飞书自建应用推送（国内可达、不依赖谷歌 FCM，2026-07-22 落地，见下「飞书自建应用推送」bullet）。用户在脚本末尾主动打印该标记串即触发（PowerShell: `Write-Host "REMOTECMD_DONE"`）。标记原样透传显示在终端（可见、可验证），不滤除。
    - **协议**：`{type:'bell', id}` 纯 S→C，单向、无 `data` 字段（事件名沿用 `bell`，仅作内部"通知触发"语义）。后端 onData 扫描到 `doneToken` 立即 broadcast；`{type:'done_token' / 'bell_sound_enabled' / 'bell_toast_enabled' / 'bell_os_enabled'}` 双向同步设置项（原 `bell_debounce_ms` 已删除）。客户端不发送 `bell`。
    - **标记扫描逻辑**（per-session 状态 `doneCarry` 跨 chunk 拼接缓冲）：**任何** onData(d) 把 `doneCarry + d` 拼成 `buf`，`buf.indexOf(doneToken)`，命中则消费掉标记、保留其后未读内容进 `doneCarry` 并立即 broadcast `bell`；未命中则 `doneCarry` 仅保留末尾最多 `doneToken.length` 字节（防无限增长）。标记跨两次输出拆开（如先 `REMOTECMD_` 再 `DONE`）仍能精确命中一次。默认 `doneToken='REMOTECMD_DONE'`，空串=不触发。**命中即触发，无去抖**（标记是显式信号，最准确即时）。
    - **后端透传**：`doneToken` 命中只触发通知，不修改/过滤 `data`；`data` 始终原样 broadcast（含标记串），前端 buffer 与后端一致，重连回放无差异（遵循"不擅自过滤 PTY 输出"）。
    - **配置字段**（`config.json`）：`doneToken`（默认 `'REMOTECMD_DONE'`，完成标记串，空串=不触发）/ `bellSoundEnabled`（默认 true）/ `bellToastEnabled`（默认 true）/ `bellBeepDurationMs`（默认 300，范围 50-2000，蜂鸣单声时长）/ `bellOsEnabled`（默认 true，系统通知 OS 弹窗开关）/ `feishuAppId`（默认 ''，飞书自建应用 app_id）/ `feishuAppSecret`（默认 ''，飞书自建应用 app_secret，仅服务端用）/ `feishuReceiveId`（默认 ''，接收消息的 ID：邮箱/chat_id/open_id/user_id，依 receive_type）/ `feishuReceiveType`（默认 'email'，接收 ID 类型：email/chat_id/open_id/user_id）。多端同步，连接时下发、设置变更时广播。
    - **设置即时生效语义**：`doneToken` 在每次 onData 实时读取当前配置值（无 `setTimeout` capture），设置变更立即对后续输出生效，不需要重启 server。
    - **前端**：复用通用 `showToast(text, 'success')`（带堆叠偏移，多终端同时 bell 不重叠），无点击行为。蜂鸣失败时（浏览器自动播放策略拒绝）静默吞错不影响 Toast。`AudioContext` 懒初始化（首次响铃时 `new`）+ 幂等 `resume()`，避免页面加载即触发自动播放警告。设置弹窗「通知」组加一行：text input「完成标记 (done token)」（placeholder `REMOTECMD_DONE`）+ Apply 按钮；即时 `wsSend({type:'done_token', data})`。
    - **系统级通知（OS Notification）升级（2026-07-21）**：在既有 Toast+蜂鸣之上，额外调用浏览器 `Notification` API 触发**操作系统弹窗**（切走标签页/最小化窗口也能收到）。`bell` 分支在 `showBellToast`+`playBeep` 之后、受独立开关 `bellOsEnabled` 门控：`fireOsNotification(id)` 仅在 `Notification.permission==='granted'` 时 `new Notification('终端通知', { body:'<终端名> 触发提示', tag:'remote-cmd-bell' })`，点击 `window.focus()` 回页面；不支持/未授权静默跳过。`Notification.requestPermission()` 在设置弹窗勾选「系统通知（OS 弹窗）」时（`applySettingsBellOsEnabled`）首次触发。仅 localhost/127.0.0.1 是 secure context 可用；局域网 HTTP 不可用（本期非目标）。
    - **ntfy 安卓系统通知推送（2026-07-22）**：在 done-token 命中后的 `bell` 事件上挂一个后端出口，触发时向 ntfy 话题 POST 一行消息，安卓 ntfy App 订阅即收系统通知。复用后端 done-token 命中触发的 `broadcast({type:'bell',id})`（命中即发，天然准确无刷屏），由服务器 `fetch()` 发起（浏览器关掉也能推）。配置项 `ntfyEnabled`（默认 false，开关）+ `ntfyTopic`（默认 ''，话题名即密码需随机不可猜）；连接时下发 `ntfy_enabled`/`ntfy_topic`，设置变更 `wsSend`→服务端 `broadcast`+`saveConfig`，多端同步。服务器侧：`broadcast({type:'bell',id})` 之后若 `config.ntfyEnabled && config.ntfyTopic` 则 `fetch('https://ntfy.sh/'+encodeURIComponent(topic), {method:'POST', headers:{'Title':'RemoteCMD 通知','Tag':'bell'}, body:'终端「<名>」任务完成'})`（fire-and-forget，失败静默）。前端设置弹窗「通知」组加一行：checkbox「Push to phone (ntfy)」即时应用 + 话题 text input + Apply 按钮；话题空则提示。默认关闭，公共 ntfy.sh 仅验证用，隐私敏感可日后自建。

    - **飞书自建应用推送（2026-07-22，替代原 webhook 方式）**：国内可达、走飞书系统推送（省电、不依赖谷歌 FCM）。后端用 `app_id`+`app_secret` 换 `tenant_access_token`（内存缓存、提前 60s 刷新）后调 `/open-apis/im/v1/messages` 发文本消息，可发到个人（email/open_id/user_id）或群（chat_id），不暴露 webhook URL、额度更高。配置项 `feishuAppId`/`feishuAppSecret`/`feishuReceiveId`/`feishuReceiveType`（默认 `email`）；连接时下发 `feishu_app_id`/`feishu_app_secret`/`feishu_receive_id`/`feishu_receive_type`，设置变更 `wsSend`→服务端 `broadcast`+`saveConfig`，多端同步。服务器侧：`broadcast({type:'bell',id})` 之后若三者齐备则 `getFeishuTenantToken()`→`sendFeishuMessage('终端「<名>」任务完成')`（fire-and-forget，失败静默）。前端设置弹窗「通知」组：app_id text、app_secret password、接收 ID text、接收类型 select（email/chat_id/open_id/user_id）+ 应用按钮；缺项提示。
