...
41. **BEL 终端响铃通知通道（2026-07-14 引入）**：后端检测 PTY 输出中的 ASCII BEL 字符 `\x07`，前端显示 `终端通知: <终端名>` Toast + 播放 1kHz 蜂鸣（默认 300ms，可配）。PowerShell 中可用 `Write-Host "`a"`、`[Console]::Write("`a")`、`` `a `` 字面量等触发。`[console]::Beep()` 直接调用 Windows 系统蜂鸣器，不走 PTY 输出，因此**不触发**该通道（设计约束）。
    - **协议**：`{type:'bell', id}` 纯 S→C，单向、无 `data` 字段。后端检测到 `\x07` 时启动/重置去抖定时器，到点未取消则 broadcast；`{type:'bell_debounce_ms' / 'bell_sound_enabled' / 'bell_toast_enabled' / 'bell_os_enabled'}` 双向同步设置项。客户端不发送 `bell`（PowerShell 进程自己输出 `\x07` 触发）。
    ...
    - **配置字段**（`config.json`）：`bellDebounceMs`（默认 1000，范围 100-10000，去抖时长）/ `bellSoundEnabled`（默认 true）/ `bellToastEnabled`（默认 true）/ `bellBeepDurationMs`（默认 300，范围 50-2000，蜂鸣单声时长）/ `bellOsEnabled`（默认 true，系统通知 OS 弹窗开关）。多端同步，连接时下发、设置变更时广播。
    ...
    - **ntfy 安卓系统通知推送（2026-07-22）**：在既有 BEL 去抖事件上挂一个后端出口，触发时向 ntfy 话题 POST 一行消息，安卓 ntfy App 订阅即收系统通知。复用后端 `bellArmed`/`bellDebounceMs` 去抖（天然防刷屏），由服务器 `fetch()` 发起（浏览器关掉也能推）。配置项 `ntfyEnabled`（默认 false，开关）+ `ntfyTopic`（默认 ''，话题名即密码需随机不可猜）；连接时下发 `ntfy_enabled`/`ntfy_topic`，设置变更 `wsSend`→服务端 `broadcast`+`saveConfig`，多端同步。服务器侧：`broadcast({type:'bell',id})` 之后若 `config.ntfyEnabled && config.ntfyTopic` 则 `fetch('https://ntfy.sh/'+encodeURIComponent(topic), {method:'POST', headers:{'Title':'RemoteCMD 通知','Tag':'bell'}, body:'终端「<名>」任务完成'})`（fire-and-forget，失败静默）。前端设置弹窗「通知」组加一行：checkbox「Push to phone (ntfy)」即时应用 + 话题 text input + Apply 按钮；话题空则提示。默认关闭，公共 ntfy.sh 仅验证用，隐私敏感可日后自建。后端零改 BEL 检测逻辑。
