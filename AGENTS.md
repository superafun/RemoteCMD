...
41. **完成标记触发通知通道（原 BEL 通道 2026-07-14 引入，2026-07-22 改为完成标记触发）**：用显式完成标记（默认 `REMOTECMD_DONE`）替换原 BEL(`\x07`) 检测作为通知触发源——BEL 被 TUI/程序自发响铃误报太多。命中后前端显示 `终端通知: <终端名>` Toast + 播放 1kHz 蜂鸣（默认 300ms，可配）+ 系统 OS 弹窗 +（可选）ntfy 手机推送。用户在脚本末尾主动打印该标记串即触发（PowerShell: `Write-Host "REMOTECMD_DONE"`）。标记原样透传显示在终端（可见、可验证），不滤除。
    ...
    - **配置字段**（`config.json`）：`doneToken`（默认 `'REMOTECMD_DONE'`，完成标记串，空串=不触发）/ `bellSoundEnabled`（默认 true）/ `bellToastEnabled`（默认 true）/ `bellBeepDurationMs`（默认 300，范围 50-2000，蜂鸣单声时长）/ `bellOsEnabled`（默认 true，系统通知 OS 弹窗开关）。多端同步，连接时下发、设置变更时广播。
